...
* @brief A simplified version of facedetect.cpp, show how to load a cascade classifier and 
* how to find objects (Face + eyes) in a video stream - Using LBP here
*/
...
int main(void)
{
	...
	if (!capWebcam.isOpened()) { printf("--(!)Error opening video capture\n"); return -1; }

	while (capWebcam.read(frame))
	{
		if (frame.empty())
		{
		...
		if (charCheckForEscKey == 27 && capWebcam.isOpened())
		{
			printf(" --(!) Stop capturing frames -- Break!");
			break;
		}
